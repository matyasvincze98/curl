...
def get_data_sources(dataset, dataset_kwargs, batch_size, test_batch_size,
    ...
    patches_128k_dict = pd.read_pickle(
        'https://wigner.hu/~fcsikor/textures/labeled_texture_oatleathersoilcarpetbubbles_subsamp1_filtered_128000_48px.pkl')
    patches_128k_train_dict = {
        key: patches_128k_dict[key] for key in ['train_images', 'train_labels']}
    patches_128k_test_dict = {
        key: patches_128k_dict[key] for key in ['test_images', 'test_labels']}

    indexes_train = random.choices(
        range(len(patches_128k_train_dict['train_images'])), k=num_train)
    indexes_test = random.choices(
        range(len(patches_128k_test_dict['test_images'])), k=num_test)

    patches_128k_train_dict['train_images'] = patches_128k_train_dict['train_images'][indexes_train].reshape(
        -1, 48, 48, 1)
    patches_128k_train_dict['train_labels'] = [
        int(item) for item in patches_128k_train_dict['train_labels'][indexes_train]]
    patches_128k_test_dict['test_images'] = patches_128k_test_dict['test_images'][indexes_test].reshape(
        -1, 48, 48, 1)
    patches_128k_test_dict['test_labels'] = [
        int(item) for item in patches_128k_test_dict['test_labels'][indexes_test]]
    ...
    full_ds = train_ds.repeat().shuffle(num_train_examples, seed=0)
    ...
    full_ds = train_ds.repeat(1).shuffle(num_train_examples, seed=0)
...
def run_training(
    ...
    # Max num of gen batches (proxy for storing a model)
    ...
    # Set up CURL modules.
    shared_encoder = model.SharedEncoder(
        name='shared_encoder', **encoder_kwargs)
    latent_encoder = functools.partial(
        model.latent_encoder_fn, n_y=n_y, n_z=n_z)
    ...
    prior_train_probs = utils.construct_prior_probs(
        batch_size, n_y, n_y_active)
    ...
    with tf.train.SingularMonitoredSession() as sess:
        ...
        for step in range(n_steps):
            ...
            refresh_on_expansion_started = (
                gen_refresh_on_expansion and has_expanded)
            ...
            if (step+1) % report_interval == 0:
                if valid_data is not None:
                    ...
                    results['valid_purity'] = compute_purity(
                        results['valid_confusion'])
                else:
                ...
                results.update(process_dataset(dataset_ops.test_iter,
                                               ...
                                               feed_dict=feed_dict,
                                               processing_ops=proc_ops))
                results['test_purity'] = compute_purity(
            ...
            if (step + 1) % report_interval == 0:
                ...
                for nval in knn_values:
                    ...
                    clf.fit(clf_train_vals['latents'],
                            clf_train_vals['labels'])
